        ...
        const user=await UserModel.findOne({email});
          if(user){
            bcrypt.compare(pass,user.pass,(_err,result)=>{
                if(result){
                    const access_token=jwt.sign({userID:user._id},"Shashank",{expiresIn:'1h'});
                    const refresh_token=jwt.sign({userID:user._id},"Shashank",{expiresIn:'2h'});

                    res.status(200).json({msg:"Login", access_token,refresh_token});
                }else{
                    res.status(200).json({msg:"Check Again"});
                }
            })
          }
    }
